...
export function findPassenger(passengers, name) {
  // Your code here
  if(!Array.isArray(passengers) || passengers.length === 0)return undefined;

  if(typeof name !== "string")return undefined;

  return passengers.find((person)=>   person.name.toLowerCase() === name.toLowerCase() )
...
export function getPassengerIndex(passengers, name) {
  ...
  return passengers.findIndex((person) => person.name.toLowerCase() === name.toLowerCase() );
...
export function isAnyWaitlisted(passengers) {
  ...
  return passengers.some((person)=> {return  person.status === "waitlisted"});
...
export function areAllConfirmed(passengers) {
  // Your code here
  if(!Array.isArray(passengers ) || passengers.length === 0)return false;
  return passengers.every((person) => {return person.status === "confirmed"})
...
export function getWaitlistedPassengers(passengers) {
  ...
  return passengers.filter((person) => {return person.status === "waitlisted"})
}
