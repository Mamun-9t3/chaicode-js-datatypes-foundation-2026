...
export function writePostcard(sender, receiver, message) {
  ...
  const senDer = sender.trim();
  const reCeiver = receiver.trim();
  const mesSage = message.trim();
  if(reCeiver.length === 0 || senDer.length === 0 || mesSage.length === 0)return "";
...
export function isValidPincode(code) {
  ...
  if (coDe.length !== 6 || coDe.startsWith("0"))return false;
  const result = /^\d+$/.test(coDe)
...
export function formatPostcardField(label, value, width) {
  ...
  const newLabel = laBel.padEnd(width);
...
export function isFromState(address, stateCode) {
  ...
  const addres = address.trim();
  const state = stateCode.trim();
  const endsWith = addres.endsWith(state);
  return endsWith;
}

// 5. countVowels(message)
//  *      - .match() se saare vowels dhundho
